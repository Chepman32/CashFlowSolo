...
export default function AddTransactionModal({
  ...
  const pickAnimStyle = useAnimatedStyle(() => ({
    transform: [{ translateX: slideAnim.value * -400 }],
    opacity: 1 - slideAnim.value,
  }));

  const formAnimStyle = useAnimatedStyle(() => ({
    transform: [{ translateX: (1 - slideAnim.value) * 400 }],
    opacity: slideAnim.value,
  }));

  const animateToPhase = (newPhase: 'pick' | 'form') => {
    const toValue = newPhase === 'form' ? 1 : 0;
  ...
  const defaultEnvelope =
    type !== 'income' ? selectedEnvelope ?? envelopes[0]?.id : selectedEnvelope;
  ...
  async function onSave() {
    ...
    const baseCurrency = useAppStore.getState().settings
      .base_currency as CurrencyCode;

    // Calculate exchange rate to base currency
    const exchangeRate = currencyService.getExchangeRate(
      selectedCurrency,
      baseCurrency,
    );
  ...
  const headerTitle =
    phase === 'pick' ? t('picker.categoryPicker') : t('addTx.title');
  ...
  async function chooseAttachment(): Promise<Attachment[]> {
    ...
    return new Promise(resolve => {
      Alert.alert(
        ...
        [
          {
            text: t('attachments.mediaLibrary'),
            onPress: async () => resolve(await pickFromMediaLibrary(t)),
          },
          {
            text: t('attachments.files'),
            onPress: async () => resolve(await pickFiles(false, t)),
          },
          {
      ...
      <SafeAreaProvider>
        ...
        >
          ...
          <View style={styles.contentContainer}>
            ...
            {/* Form Phase */}
            <Animated.View
              style={[styles.phaseContainer, formAnimStyle]}
            ...
            >
              <View style={[styles.card, { backgroundColor: theme.surface }]}>
                {selectedEnvelope && (
                  ...
                  >{`${t('common.category')}: ${
                    envelopes.find(e => e.id === selectedEnvelope)?.name
                  }`}</Text>
                )}
                <Text style={{ color: theme.textSecondary }}>
                  {t('addTx.amount')}
                ...
                >
                  <Pressable
                    onPress={async () => {
                      const items = await chooseAttachment();
                      if (items.length)
            ...
            <View style={styles.footer}>
              ...
              <Pressable
                ...
                style={[
                  ...
                  styles.buttonLarge,
                  {
                    backgroundColor: canSave ? colors.light.primary : '#94A3B8',
                  },
                ]}
              >
                <Text
    ...
    <Animated.View style={[aStyle]}>
      <Pressable
        ...
        style={[
          styles.chip,
          {
            backgroundColor: active ? colors.light.primary : 'transparent',
            borderColor,
          },
        ]}
      >
...
const getSuggestedCategories = (t: (key: string) => string) => [
  { name: t('categories.groceries'), icon: '🛒', color: '#22C55E' },
  { name: t('categories.rentUtilities'), icon: '🏠', color: '#6366F1' },
  { name: t('categories.restaurants'), icon: '🍽️', color: '#F97316' },
  { name: t('categories.transport'), icon: '🚗', color: '#0EA5E9' },
  { name: t('categories.health'), icon: '⚕️', color: '#EF4444' },
  ...
  { name: t('categories.travel'), icon: '✈️', color: '#0EA5E9' },
  { name: t('categories.gifts'), icon: '🎁', color: '#3B82F6' },
  { name: t('categories.insurance'), icon: '🛡️', color: '#64748B' },
  { name: t('categories.pets'), icon: '🐾', color: '#F59E0B' },
  { name: t('categories.personalCare'), icon: '💅', color: '#EC4899' },
  { name: t('categories.fitness'), icon: '💪', color: '#10B981' },
  { name: t('categories.home'), icon: '🔧', color: '#8B5CF6' },
  { name: t('categories.childcare'), icon: '👶', color: '#F472B6' },
  { name: t('categories.savings'), icon: '🐖', color: '#14B8A6' },
];

const getSuggestedIncomeCategories = (t: (key: string) => string) => [
  { name: t('categories.earnedIncome'), icon: '💼', color: '#22C55E' },
  { name: t('categories.freelance'), icon: '💻', color: '#6366F1' },
  ...
  { name: t('categories.bonus'), icon: '🎉', color: '#F59E0B' },
  { name: t('categories.pension'), icon: '🏦', color: '#3B82F6' },
  { name: t('categories.governmentBenefits'), icon: '🏛️', color: '#64748B' },
  { name: t('categories.giftsReceived'), icon: '🎁', color: '#EC4899' },
  { name: t('categories.refunds'), icon: '💰', color: '#10B981' },
...
function CategoryGrid({
  ...
  envelopes
    ...
    .forEach(e =>
      tiles.push({
        key: e.id,
        id: e.id,
        label: getTranslatedEnvelopeName(e.name, t),
        icon: e.icon,
        color: e.color,
      }),
    );
  ...
  const suggestions =
    type === 'income'
      ? getSuggestedIncomeCategories(t)
      : getSuggestedCategories(t);
  suggestions
...
async function pickFiles(imagesOnly = false, t?: (key: string) => string) {
  ...
  try {
    ...
    Alert.alert(
      t
        ? t('attachments.attachmentPickerNotInstalled')
        : 'Attachment Picker Not Installed',
      t
        ? t('attachments.attachmentPickerNotInstalledBody')
        : 'Attachment picker not installed',
    );
  ...
  try {
    ...
    try {
      ...
      Alert.alert(
        t ? t('attachments.unableToAttachFile') : 'Unable to attach file',
        err2?.message ||
          err?.message ||
...
async function pickFromMediaLibrary(
  t?: (key: string) => string,
): Promise<Attachment[]> {
  return new Promise(resolve => {
    const options: any = {
